...
function pastelFromKey(key: string, isDark = false) {
  let h = 0;
  for (let i = 0; i < key.length; i++) h = (h * 31 + key.charCodeAt(i)) >>> 0;
  const hue = h % 360;
  if (isDark) {
    return `hsl(${hue} 30% 15%)`; // Colores más oscuros para modo dark
  }
...
export default function NotesList({
  ...
  if (empty) {
    return (
      <div className="rounded-2xl border border-dashed border-zinc-300 bg-white p-8 text-center dark:border-zinc-600 dark:bg-zinc-800">
      ...
      {sorted.map((note) => {
        ...
        return (
          <div
            key={note.id}
            className={cn(
              "group rounded-2xl border border-zinc-200 p-4 shadow-sm transition hover:border-zinc-300 dark:border-zinc-700 dark:hover:border-zinc-600",
          ...
          >
            ...
            {thumbs.length ? (
              <div className="mb-3 overflow-hidden rounded-xl border border-zinc-200 bg-white/70 dark:border-zinc-600 dark:bg-zinc-800/70">
                <div className="grid grid-cols-3 gap-[1px] bg-zinc-200 dark:bg-zinc-600">
